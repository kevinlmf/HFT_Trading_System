...
class MicrostructureProfiler:
    ...
    def analyze(self, market_data: Dict[str, Any]) -> Dict[str, Any]:
        """
        High-level entrypoint used by higher-level pipelines (e.g. complete-flow).

        It inspects the available fields in ``market_data`` (prices, LOB, trades, etc.)
        and runs the appropriate profiling functions, returning a nested dict of
        metrics that can be serialized to JSON.
        """
        from dataclasses import asdict

        results: Dict[str, Any] = {}

        # 1) Price formation metrics
        prices = market_data.get("prices")
        if prices is None:
            # Fallback: try to reconstruct from returns if only returns are present
            returns = market_data.get("returns")
            if returns is not None and len(returns) > 0:
                base = 100.0
                prices = base * (1 + returns).cumprod()
        if isinstance(prices, pd.Series) and len(prices) > 3:
            try:
                pf = self.profile_price_formation(prices=prices)
                results["price_formation"] = asdict(pf)
            except Exception as e:
                results["price_formation_error"] = str(e)

        # 2) Liquidity structure metrics (requires LOB)
        ...
        if (
            isinstance(bid_prices, pd.Series)
            and isinstance(ask_prices, pd.Series)
            and isinstance(bid_sizes, pd.Series)
            and isinstance(ask_sizes, pd.Series)
            and len(bid_prices) > 0
        ):
            try:
                liq = self.profile_liquidity_structure(
                    bid_prices=bid_prices,
                    ask_prices=ask_prices,
                    bid_sizes=bid_sizes,
                    ask_sizes=ask_sizes,
                )
                results["liquidity_structure"] = asdict(liq)
            except Exception as e:
                results["liquidity_structure_error"] = str(e)

        # 3) Order-flow dynamics (if trades are available)
        trades = market_data.get("trades")
        if isinstance(trades, pd.DataFrame) and not trades.empty:
            try:
                of = self.profile_order_flow_dynamics(trades=trades, orders=None)
                results["order_flow_dynamics"] = asdict(of)
            except Exception as e:
                results["order_flow_dynamics_error"] = str(e)

        # Note: market impact / latency analysis can be added similarly when
        # sufficient intraday LOB & message-level data is available.
    ...
    def generate_economic_insights(self) -> Dict[str, str]:
        ...
        # 兼容 dataclass 对象和 dict 结构
        def _get(container, name: str, default: float = 0.0) -> float:
            if container is None:
                return default
            if hasattr(container, name):
                return getattr(container, name)
            if isinstance(container, dict):
                return float(container.get(name, default) or 0.0)
            return default
        ...
        # liquidity structure（注意：analyze 中的 key 是 liquidity_structure）
        liq_key = 'liquidity'
        if 'liquidity_structure' in self.profiling_results:
            liq_key = 'liquidity_structure'
        if liq_key in self.profiling_results:
            liq = self.profiling_results[liq_key]
            depth_imb = _get(liq, 'depth_imbalance', 0.0)
            resil = _get(liq, 'resiliency', 0.0)

            if abs(depth_imb) > 0.3:
                insights['liquidity_imbalance'] = f"深度不平衡显著（{depth_imb:.2f}），买卖盘力量不均"
            
            if resil > 0.5:
                insights['liquidity_resiliency'] = "流动性恢复速度快，市场效率较高"
        
        # order-flow dynamics（注意：analyze 中的 key 是 order_flow_dynamics）
        of_key = 'order_flow'
        if 'order_flow_dynamics' in self.profiling_results:
            of_key = 'order_flow_dynamics'
        if of_key in self.profiling_results:
            of = self.profiling_results[of_key]
