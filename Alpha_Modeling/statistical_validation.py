...
class StatisticalValidator:
    ...
    # 兼容 run_trading.sh 中的简单接口：
    # validator.validate(factors, forward_returns)
    def validate(
        self,
        ...
        返回一个 dict，key 是因子名，value 是 comprehensive_validation 的结果。
        """
        results: Dict[str, Any] = {}
        if forward_returns is None or len(forward_returns) == 0:
            return results

        # 简化版：目前没有逐因子时间序列，这里用统一的 returns，
        # 并假设每个因子的截面值都已经体现在 returns 的相关性里。
        # 将来如果有真正的 factor time-series，可以在这里替换为真实数据。
        for hyp in factors:
            # 构造一个简单的“因子值”序列：用 forward_returns 做 proxy
            factor_series = forward_returns.copy()
            res = self.comprehensive_validation(factor_series, forward_returns, hyp)
            results[hyp.name] = res
    ...
    def single_factor_regression(
        ...
        # 信息系数（IC）- 两个Series之间的相关性
        ic_mean = aligned_data['factor'].corr(aligned_data['return'])
        if pd.isna(ic_mean):
            ic_mean = 0.0
        # IC的标准差（简化：使用滚动窗口计算IC序列的标准差）
        if len(aligned_data) > 20:
            rolling_ic = aligned_data['factor'].rolling(window=20).corr(aligned_data['return'])
            ic_std = rolling_ic.std() if not rolling_ic.empty and not rolling_ic.isna().all() else 0.0
        else:
            ic_std = 0.0
